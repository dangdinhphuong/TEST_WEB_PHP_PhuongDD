    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-5 col-md-8 align-item-center">
                <div class="border">
                <h3 class="bg-gray p-4">Edit category</h3>
                    <form action="{{route('admin.categories.update',['id'=>$Categories->id])}}" method="post">
                    @csrf
                        <fieldset class="p-4">
                            <input type="text" placeholder="Category Name" name="name" value="{{$Categories->name}}" class="border p-3 w-100 my-2">
                            <code> {{ $errors->first('name') }} </code>
                            <div class="loggedin-forgot">
                                    <input type="checkbox" name="status" id="keep-me-logged-in" {{$Categories->status==\App\Common\Constants::ENABLED? "checked" :''}} >
